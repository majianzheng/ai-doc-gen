...
const res = await client.callTool({
  ...
  arguments: { username: 'zhangsan', filename: 'MCP测试', title: 'MCP Test', paragraphs: [{ text: 'Hello from MCP client', level: 1 }, { text: 'bullet', bullet: true }] },
});
console.log('CALL RESULT:', JSON.parse(res.content[0].text));

const res2 = await client.callTool({
  name: 'generate_pdf',
  arguments: { username: 'zhangsan', filename: 'MCP_PDF', title: 'MCP PDF', paragraphs: [{ text: 'pdf body' }] },
});
